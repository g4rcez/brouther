...
import { useQueryStringState } from "../exports";
...
export default function BroutherPage() {
    ...
    return (
        <DocumentPage title="Brouther">
            <p>
                If you need to restore the scroll between page transitions, you can use the{" "}
                <InlineCode>{"<Scroll />"}</InlineCode> component. This component enable you to restore the scroll
                position, based in the last position of screen.
            </p>
            <p>This component also enable the scroll based in anchors.</p>
            <Code code={code} />
            <button onClick={() => setQs({ math: Math.random() })}>Random</button>
        </DocumentPage>
    );
}
