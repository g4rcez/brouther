...
export type FetchPaths<Routes extends Route[]> = NonNullable<
  { [_ in keyof Routes[number]]: Routes[number]["path"] }["path"]
>;

export type CreateHref<T extends Route[]> = <
...
export type CreateMappedRoute<_Router extends Function.Narrow<Router>> = {
  ...
  link: <
    ...
    const QS extends QueryString.Has<Path> extends infer ExistQs ? QueryString.Parse<Path> : Readonly<Paths.Variables<Paths.Pathname<Path>>>,
