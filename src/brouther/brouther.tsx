...
export const Brouther = <T extends Base>({ config, cacheSize = 5, flags, ErrorElement, children, filter }: BroutherProps<T>) => {
    ...
    useEffect(() => {
        ...
        if (result.page === null) RouteEvents.notFound(href);
        if (!(loader && page)) {
            if (result.page !== null) RouteEvents.change(href);
        ...
        return void request().then((response) => {
            ...
            setState((prev) => {
                ...
                if (cache.size >= cacheSize) Array.from(cache.keys()).forEach((k, i) => (i < cacheSize ? cache.delete(k) : undefined));
                return {
                    ...prev,
                    matches: result,
                    firstLoading: false,
