...
export const keys = <T extends {}>(o: T): Array<keyof T> =>
  Object.keys(o) as any;

export const encodeTuple = (key: string, val: string | null) => {
  const v = val?.toString();
  return `${encodeURIComponent(key.toString())}=${
    v === undefined ? "" : v === null ? null : encodeURIComponent(v)
  }`;
};

const replaceUrlParams = <Path extends string, Keys extends UrlParams<Path>>(
  path: string,
  keys: Keys | undefined
) =>
  keys === undefined
    ? path
    : path.replace(/:(\w+)/g, (_, b) => `${(keys as any)[b]}`);
    ...
    : keys(o)
        .reduce<string[]>((acc, key) => {
          ...
          return Array.isArray(val)
            ? [
                ...acc,
                ...val.map((x) => encodeTuple(key as string, x as never)),
              ]
...
export const createHref = (
  ...
  return u.href
    .replace(u.origin, "")
    .replace(new RegExp(`^/${basename}`, "gi"), "/");
};
...
export const transformData = <T extends {}>(map: string, o: Data): T => {
  const object: any = {};
  o.forEach((v, key) => {
    const value = parse(v);
